...
#define FOR1(a) for (int i = _; i < (a); i++)
...
what is the chance that the people in the memorial got a kill before they died

*/

main() {
    ri(n, k);
    db expected = db (db (n) / 2.0 - 1) / (n - 1);
    db sum = 0;
    FOR (_, k) {
        vpi edges;
        FOR (i, n) {
